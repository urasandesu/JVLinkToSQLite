...
{
    public class JVWatchEventCloseResult : JVLinkResult
    {
        protected override void SetReturnCodeCore(int returnCode)
        {
            if (returnCode == 0)
            {
                ...
                DebugMessage = $"[{nameof(JVLink.JVWatchEventClose)}]正常 。RC={returnCode}";
                DebugCauseAndTreatment = "-";
            }
            else
            {
                SetUnknownReturnCode(nameof(JVLink.JVWatchEventClose), returnCode);
            }
        }
    }
}
